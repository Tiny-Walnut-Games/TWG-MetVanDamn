	...
	public class BiomeFieldSystemTests
		...
		private Entity CreateBiomeEntity(Polarity primary, int2 coords, byte level = 0)
			...
			em.AddComponentData(e,
				new CoreBiome
					{
					Type = BiomeType.Unknown, PrimaryPolarity = primary, SecondaryPolarity = Polarity.None,
					PolarityStrength = 0f, DifficultyModifier = 0f
					});
		...
		public void PolarityStrength_CenterWeaker_OuterStronger()
			...
			Assert.Less(centerBiome.PolarityStrength, edgeBiome.PolarityStrength,
				"Center polarity strength should be lower than far edge.");
			}
		...
		public void PolarityStrength_LevelInfluence_IncreasesWithLevel()
			...
			Assert.Greater(highBiome.PolarityStrength, lowBiome.PolarityStrength,
				"Higher level should amplify polarity strength.");
			}
		...
		public void SecondaryPolarity_OnlyForTransitionZone()
			...
			if (biome.Type == BiomeType.TransitionZone)
				{
				Assert.AreNotEqual(Polarity.None, biome.SecondaryPolarity,
					"Transition zone should gain secondary polarity.");
				}
			else
				{
				Assert.AreEqual(Polarity.None, biome.SecondaryPolarity,
					"Non-transition biomes should have no secondary polarity.");
				}
			}
		...
		public void DifficultyModifier_Smoothing_IncreasesOverUpdates()
			...
			Assert.GreaterOrEqual(second, first,
				"Difficulty modifier should not decrease across early smoothing updates.");
			}
		}
	}
