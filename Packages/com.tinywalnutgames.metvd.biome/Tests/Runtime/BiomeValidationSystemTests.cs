...
namespace TinyWalnutGames.MetVD.Tests.Biome
{
    ...
    public class BiomeValidationSystemTests
    {
        ...
        {
            var e = CreateBiomeEntity(new CoreBiome { Type = BiomeType.HubArea, PrimaryPolarity = Polarity.Sun, PolarityStrength = 0.5f, SecondaryPolarity = Polarity.None, DifficultyModifier = 1f }, new int2(0,0));
            _simGroup.Update();
            var em = _world.EntityManager;
        ...
        {
            var e = CreateBiomeEntity(new CoreBiome { Type = BiomeType.TransitionZone, PrimaryPolarity = Polarity.None, SecondaryPolarity = Polarity.Moon, PolarityStrength = 0.2f, DifficultyModifier = 1f }, new int2(2,2));
            _simGroup.Update();
        ...
        public void BiomeTypeAssignment_Mismatch_AddsRecord()
        {
            ...
            var e = CreateBiomeEntity(new CoreBiome { Type = BiomeType.SolarPlains, PrimaryPolarity = Polarity.Moon, PolarityStrength = 0.5f, DifficultyModifier = 1f }, new int2(0,10));
            _simGroup.Update();
            var buf = _world.EntityManager.GetBuffer<BiomeValidationRecord>(e);
        ...
        {
            var e = CreateBiomeEntity(new CoreBiome { Type = BiomeType.HubArea, PrimaryPolarity = Polarity.Sun, PolarityStrength = 0.5f, DifficultyModifier = 4.5f }, new int2(1,1));
            _simGroup.Update();
            var buf = _world.EntityManager.GetBuffer<BiomeValidationRecord>(e);
