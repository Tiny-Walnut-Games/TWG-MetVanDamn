...
namespace TinyWalnutGames.MetVD.QuadRig.Tests
	...
	public class QuadRigHumanoidTests
		...
		public void BillboardUtility_ValidatesBillboardConfig()
			...
			var invalidBillboard = new BillboardData(); // Default is valid, so we modify to make invalid
			invalidBillboard.IsActive = false; // Inactive billboard is considered invalid for this test

			// Act & Assert
			Assert.That(BillboardUtility.ValidateBillboardConfig(validBillboard), Is.True);
			Assert.That(BillboardUtility.ValidateBillboardConfig(invalidBillboard), Is.False);
			}
		...
		public void BoneHierarchyUtility_ValidatesHierarchyCorrectly()
			...
			var invalidHierarchy = new BoneHierarchyElement[2]
				{
				new(0, 1, float3.zero, quaternion.identity, 0), // Circular reference
				new(1, 0, float3.zero, quaternion.identity, 0)
				};
			BoneHierarchyElement[]? nullOrEmptyHierarchy = null;
			...
			Assert.That(BoneHierarchyUtility.ValidateBoneHierarchy(nullOrEmptyHierarchy!), Is.False);
