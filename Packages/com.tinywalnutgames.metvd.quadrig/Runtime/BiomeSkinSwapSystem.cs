...
// Removed obsolete Hybrid Renderer usage; using managed mesh/material holder instead.
...
namespace TinyWalnutGames.MetVD.QuadRig
    {
    // 🧙‍♂️ SACRED SKIN SWAP ENGINE: Biome adaptation system preserving visual integrity during environmental transitions
    // 🔧 ENHANCEMENT READY: Material atlas system for efficient texture swapping across biomes
    // 🍑 CHEEK PRESERVATION: Protects against visual corruption during rapid biome transitions
    ...
    public partial class BiomeSkinSwapSystem : SystemBase
        ...
        private Dictionary<uint, Material> atlasIdToMaterial = new();
        ...
        protected override void OnCreate()
            ...
            skinSwapQuery = GetEntityQuery(
                ...
                ComponentType.ReadOnly<QuadRigMeshMaterial>()
            );
        ...
        protected override void OnDestroy()
            {
            // Dictionary cleanup not required explicitly; GC handles it.
            }
        ...
        protected override void OnUpdate()
            ...
            Entities
                ...
                .ForEach((Entity entity, ref QuadRigHumanoid humanoid, in TextureAtlasData atlasData) =>
                {
                    if (humanoid.CurrentAtlasId != atlasData.AtlasId)
                        {
                        SwapSkin(entity, ref humanoid, atlasData);
                        }
                })
                .WithStructuralChanges() // Access managed component objects
                .Run();
            }
        ...
        /// </summary>
        private void SwapSkin(Entity entity, ref QuadRigHumanoid humanoid, in TextureAtlasData atlasData)
            {
