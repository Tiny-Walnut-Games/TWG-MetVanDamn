...
namespace LivingDevAgent.Editor.Modules
{
    ...
    public class NavigatorModule : ScribeModuleBase
    {
        ...
        void DrawFileNode(string filePath, string indent, int depth)
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUI.indentLevel++;
        ...
        void DrawImageThumbnail(string filePath)
        {
        ...
        void DrawFileActionButtons(string filePath, bool isImage)
        {
            if (!isImage)
            {
                if (GUILayout.Button("📖", EditorStyles.miniButton, GUILayout.Width(25)))
        ...
        void CopyImageMarkdownLink(string filePath)
        {
        ...
        void InsertImageAtCursor(string filePath)
        {
            string baseDir = string.IsNullOrEmpty(_data.CurrentFilePath) ? ResolveActiveFolder() : Path.GetDirectoryName(_data.CurrentFilePath);
            if (string.IsNullOrEmpty(baseDir))
            {
        ...
        GUIStyle _navBackgroundStyle;

        void InitializeStyles()
        {
            if (_navBackgroundStyle != null) return;

            // 🎨 FIXED: Subtle, darker border that doesn't compete with content
            _navBackgroundStyle = new GUIStyle("Box")
            {
                ...
                border = new RectOffset(2, 1, 1, 1), // Reduced from 3 to 2
        ...
        Texture2D CreateNavBackgroundTexture()
        {
            ...
            var bgColor = EditorGUIUtility.isProSkin ? new Color(0.22f, 0.22f, 0.22f, 1f) : new Color(0.85f, 0.85f, 0.85f, 1f);
            // 🎨 FIXED: Much more subtle border - darker, less saturated
            var borderColor = EditorGUIUtility.isProSkin ? new Color(0.35f, 0.4f, 0.45f, 1f) : new Color(0.6f, 0.65f, 0.7f, 1f);
            ...
            for (int y = 0; y < 32; y++)
            {
                for (int x = 0; x < 32; x++)
                {
                    if (x < 2) // Left border - much more subtle
