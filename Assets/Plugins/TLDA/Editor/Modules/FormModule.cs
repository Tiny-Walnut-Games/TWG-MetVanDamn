...
using UnityEditor;
using UnityEngine;
using LivingDevAgent.Editor; // Add using for ScribeUtils
...
namespace LivingDevAgent.Editor.Modules
{
    ...
    {
        private bool _shouldSwitchToEditor = false;
        private bool _shouldSwitchToPreview = false;

        public bool ShouldSwitchToEditor => _shouldSwitchToEditor;
        public bool ShouldSwitchToPreview => _shouldSwitchToPreview;

        public FormModule(TLDLScribeData data) : base(data) { }

        public void ResetSwitchFlags()
        {
            _shouldSwitchToEditor = false;
            _shouldSwitchToPreview = false;
        }
        ...
        public void DrawToolbar()
        {
            ...
            {
                var elapsed = System.DateTime.Now - _data.SessionStartTime;
        ...
        void DrawTechnicalDetailsSection()
        {
            EditorGUILayout.BeginVertical("box");
        ...
        void DrawTimeTrackingSection()
        {
            ...
            if (_data.TimeSessions.Count > 0)
            {
                ...
                var lastSession = _data.TimeSessions[_data.TimeSessions.Count - 1];
        ...
        void ShowSessionHistory()
        {
            ...
            foreach (var session in _data.TimeSessions)
            {
        ...
        string BuildMarkdown()
        {
            ...
            if (_data.IncludeMetadata)
            {
                ...
                if (_data.TimeSessions.Count > 0)
                {
                    ...
                    if (_data.IsTimerActive)
                    {
                        var currentElapsed = (System.DateTime.Now - _data.SessionStartTime).TotalMinutes;
        ...
        string ResolveActiveFolder()
        {
            if (!string.IsNullOrEmpty(_data.ActiveDirPath))
                return _data.ActiveDirPath;
            if (!string.IsNullOrEmpty(_data.RootPath))
                return _data.RootPath;
            return System.IO.Path.Combine(UnityEngine.Application.dataPath, "Plugins/TLDA/docs");
        }

        string MakeProjectRelative(string absPath)
        {
            string projectRoot = System.IO.Directory.GetParent(UnityEngine.Application.dataPath)!.FullName.Replace('\\', '/');
