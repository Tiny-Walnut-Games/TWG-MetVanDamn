	...
	public class BiomeRegionExtractor : EditorWindow
		...
		private void DetectBiomesFromJSON(TextAsset jsonAsset)
			...
			try
				...
				if (jsonData != null && jsonData.biomes != null)
					...
					{
					Debug.LogWarning(
						"Invalid JSON format. Expected format: {\"biomes\": [{\"name\": \"Forest\", \"color\": \"#00FF00\"}]}");
					}
				}
			catch (Exception e)
				{
		...
		private Color ParseColorFromHex(string hexColor)
			...
			if (hexColor.Length == 6)
				...
				if (int.TryParse(hexColor[..2], System.Globalization.NumberStyles.HexNumber, null, out int r) &&
				    int.TryParse(hexColor.Substring(2, 2), System.Globalization.NumberStyles.HexNumber, null,
					    out int g) &&
				    int.TryParse(hexColor.Substring(4, 2), System.Globalization.NumberStyles.HexNumber, null,
					    out int b))
					{
		...
		private void RunPreFlightValidation()
			...
			foreach (Texture2D sheet in spritesheets)
				...
				if (cellSize.x > sheet.width || cellSize.y > sheet.height)
					...
					validationWarnings.Add(
						$"Cell size ({cellSize.x}x{cellSize.y}) is larger than spritesheet '{sheet.name}' ({sheet.width}x{sheet.height}).");
					}
				}
			...
			if (string.IsNullOrEmpty(fileNamingPattern))
				...
				{
				validationWarnings.Add(
					"File naming pattern should include {biome} placeholder for proper organization.");
				}
			...
			IEnumerable<string> duplicates =
				biomeNames.GroupBy(name => name).Where(g => g.Count() > 1).Select(g => g.Key);
			foreach (string duplicate in duplicates)
				{
				validationWarnings.Add(
					$"Duplicate biome name detected: '{duplicate}'. Each biome must have a unique name.");
				}
		...
		private void ProcessBiomeExtraction()
			...
			Debug.Log(
				$"✓ Biome extraction completed. Processed {biomesToProcess.Count} biomes across {spritesheets.Count} spritesheets.");
			}
		...
		private void SliceSpritesheetWithBiomeFilter(Texture2D spritesheet, Texture2D maskTexture, BiomeMapping biome,
			string outputFolder)
			{
			string spritesheetPath = AssetDatabase.GetAssetPath(spritesheet);
			TextureImporter? importer = AssetImporter.GetAtPath(spritesheetPath) as TextureImporter;
			...
			try
				...
				if (biomeSprites.Count > 0)
					...
					{
					Debug.LogWarning(
						$"No sprites found for biome '{biome.biomeName}' in spritesheet '{spritesheet.name}'.");
					}
				}
			finally
				{
		...
		private void CreateBiomeSpecificTexture(Texture2D sourceTexture, List<SpriteRect> sprites, BiomeMapping biome,
			string outputFolder)
			{
			...
			if (newImporter != null)
				...
				Debug.Log(
					$"Created biome texture '{newTextureName}' with {sprites.Count} sprites for biome '{biome.biomeName}'.");
				}
			}
