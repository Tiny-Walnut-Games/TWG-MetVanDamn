...
namespace TinyWalnutGames.Tools.Editor
	...
	public class BatchSpriteSlicer : EditorWindow
		...
		/// </summary>
		[MenuItem("Tiny Walnut Games/Batch Sprite Slicer")] // Moved under Tiny Walnut Games root
		...
		private void CopySlicesFromSelected()
			...
			ISpritePhysicsOutlineDataProvider outlineProvider =
			...
			Debug.Log(
				$"Copied {rects.Count} sprite rects (and outlines) from '{texture.name}' ({copiedTexWidth}x{copiedTexHeight}).");
			}
		...
		private void PasteSlicesToSelected()
			...
			foreach (Object obj in selectedTextures)
				...
				foreach (SpriteRect srcRect in copiedRects)
					...
					if (copiedOutlines.TryGetValue(srcRect.spriteID.ToString(), out List<Vector2[]> outlines) &&
				...
				Debug.Log(
					$"Pasted {newRects.Count} slices (and outlines) to '{texture.name}' ({texWidth}x{texHeight}).");
				}
			}
		...
		private void AdjustPivotOfSelectedSlices()
			...
			Object[] selectedTextures =
				Selection.GetFiltered(typeof(Texture2D),
					SelectionMode.Assets); // Get all selected textures in the project
			...
			foreach (Object obj in selectedTextures)
				...
				TextureImporter?
					importer =
						AssetImporter
							.GetAtPath(path) as TextureImporter; // Get the TextureImporter for the selected texture
				...
				var factory =
					new SpriteDataProviderFactories(); // Create a new instance of SpriteDataProviderFactories to access sprite data providers
				...
				ISpriteEditorDataProvider
					dataProvider =
						factory.GetSpriteEditorDataProviderFromObject(
							importer); // Get the sprite editor data provider for the texture importer
				dataProvider
					.InitSpriteEditorDataProvider(); // Initialize the sprite editor data provider to access sprite data

				var rects = new List<SpriteRect>(dataProvider
					.GetSpriteRects()); // Get all sprite rectangles from the data provider
		...
		private void SliceSelectedSprites()
			...
			foreach (Object obj in selectedTextures)
				...
				Texture2D
					texture = AssetDatabase.LoadAssetAtPath<Texture2D>(path); // Load the texture asset from the path
				...
				if (useCellSize)
					{
					spriteWidth =
						Mathf.Max(1, Mathf.RoundToInt(cellSize.x)); // Calculate sprite width based on cell size
					spriteHeight =
						Mathf.Max(1, Mathf.RoundToInt(cellSize.y)); // Calculate sprite height based on cell size
					actualColumns =
						Mathf.Max(1,
							texWidth / spriteWidth); // Calculate actual columns based on texture width and sprite width
					actualRows =
						Mathf.Max(1,
							texHeight /
							spriteHeight); // Calculate actual rows based on texture height and sprite height
					}
				else
					{
					...
					spriteWidth =
						texWidth / actualColumns; // Calculate sprite width based on texture width and actual columns
					spriteHeight =
						texHeight / actualRows; // Calculate sprite height based on texture height and actual rows
					}
				...
				TextureImporter
					texImporter =
						(TextureImporter)TextureImporter.GetAtPath(path); // Get the TextureImporter for the texture
				...
				dataProvider.SetSpriteRects(spriteRects
					.ToArray()); // Set the created sprite rectangles to the data provider
