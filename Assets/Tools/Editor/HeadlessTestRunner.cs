	...
	public static class HeadlessTestRunner
		...
		private static void RunTestsInternal(TestMode testMode)
			...
			if (string.IsNullOrEmpty(resultsPath))
				{
				Debug.LogError("HeadlessTestRunner: No valid results path specified");
				return;
				}
			...
			var filter = new Filter
				{
				testMode = testMode,
				testNames = null, // Temporarily disable filtering to see all tests
				groupNames = null,
				categoryNames = null, // Temporarily disable category filtering
				assemblyNames = null
				};
			// Temporarily disable test name filtering
		...
		private static void ParseArgs(out string? resultsPath, out string[]? testNames, out string[]? categories)
			...
			try
				...
				for (int i = 0; i < args.Length; i++)
					...
					if (string.Equals(a, "-testFilter", StringComparison.OrdinalIgnoreCase) ||
					    string.Equals(a, "-filter", StringComparison.OrdinalIgnoreCase))
						{
						if (i + 1 < args.Length)
							{
							string raw = args[++i];
					...
					if (string.Equals(a, "-testCategory", StringComparison.OrdinalIgnoreCase) ||
				...
				{
				/* ignore parse issues; fall back to defaults */
				}
			}
		...
		private class Callbacks : ICallbacks
			...
			private static void WriteEnhancedNUnitXml(ITestResultAdaptor result, string path, DateTime startTime,
				DateTime endTime, TestMode testMode)
				{
				...
				sb.AppendFormat(
					"<test-run id=\"1\" testcasecount=\"{0}\" total=\"{0}\" passed=\"{1}\" failed=\"{2}\" inconclusive=\"{3}\" skipped=\"{4}\" result=\"{5}\" start-time=\"{6}\" end-time=\"{7}\" duration=\"{8:F6}\">\n",
				...
				sb.AppendFormat("  <command-line><![CDATA[Unity {0} Tests via HeadlessTestRunner]]></command-line>\n",
					testMode);
				sb.Append(
					"  <test-suite type=\"Assembly\" id=\"0-1000\" name=\"Unity.TestRunner\" fullname=\"Unity.TestRunner\">\n");
				...
				if (node.HasChildren)
					...
					foreach (var child in node.Children)
						...
						sb.AppendFormat("{0}  <result>{1}</result>\n", indent,
							child.FailCount == 0 ? "Passed" : "Failed");
			...
			private static void CountResults(ITestResultAdaptor node, ref int passed, ref int failed, ref int skipped,
				ref int inconclusive)
				{
				if (node.HasChildren)
					{
