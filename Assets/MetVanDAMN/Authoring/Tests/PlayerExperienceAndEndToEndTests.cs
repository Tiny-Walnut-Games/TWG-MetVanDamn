...
namespace TinyWalnutGames.MetVD.Graph.Tests
	...
	public class PlayerExperienceAndEndToEndTests
		...
		public IEnumerator EndToEndWorldGeneration_ProducesPlayableWorld_WithValidProgression()
			...
			EntityQuery roomQuery = _entityManager.CreateEntityQuery(typeof(RoomHierarchyData));
			EntityQuery gateQuery = _entityManager.CreateEntityQuery(typeof(GateCondition));
		...
		public void SaveLoadSystem_PreservesWorldState_AcrossSessionSimulation()
			...
			WorldConfiguration savedWorldConfig = _entityManager.GetComponentData<WorldConfiguration>(worldStateEntity);
			RoomHierarchyData savedRoom1Data = _entityManager.GetComponentData<RoomHierarchyData>(room1);
			RoomHierarchyData savedRoom2Data = _entityManager.GetComponentData<RoomHierarchyData>(room2);

			// Simulate session restart by modifying data
			WorldConfiguration modifiedConfig = originalWorldConfig;
			...
			WorldConfiguration restoredWorldConfig = _entityManager.GetComponentData<WorldConfiguration>(worldStateEntity);
			RoomHierarchyData restoredRoom1Data = _entityManager.GetComponentData<RoomHierarchyData>(room1);
			RoomHierarchyData restoredRoom2Data = _entityManager.GetComponentData<RoomHierarchyData>(room2);
		...
		public void ErrorHandling_RecoverGracefully_FromCorruptedData()
			...
			try
				...
				WorldConfiguration processedConfig = ValidateAndSanitizeWorldConfig(corruptedConfig);
		...
		private bool CanPlayerAccessArea(Ability playerSkills, Entity gateEntity, Entity areaEntity)
			{
			GateCondition gateCondition = _entityManager.GetComponentData<GateCondition>(gateEntity);
			MovementCapabilityTags areaRequirements = _entityManager.GetComponentData<MovementCapabilityTags>(areaEntity);
		...
		private WorldConfiguration ValidateAndSanitizeWorldConfig(WorldConfiguration config)
			{
			WorldConfiguration sanitized = config;
