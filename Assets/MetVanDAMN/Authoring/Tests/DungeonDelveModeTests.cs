...
namespace TinyWalnutGames.MetVanDAMN.Authoring.Tests
	...
	public class DungeonDelveModeTests
		...
		public void SetUp()
			...
			// Set as default world for Unity ECS
			...
			dungeonMode.SetEntityManagerForTesting(entityManager);
...
#nullable enable
		...
		public void DungeonDelveMode_InitializesCorrectly()
			...
			Assert.AreEqual(DungeonDelveState.NotStarted, dungeonMode.CurrentState,
		...
		[UnityTest]
		public IEnumerator DungeonDelveMode_CanStartDungeon()
			{
			// Act
			...
			// Assert
			Assert.AreNotEqual(DungeonDelveState.NotStarted, dungeonMode.CurrentState,
				"State should change from NotStarted");
			Assert.IsTrue(dungeonMode.SessionDuration >= 0, "Session duration should be non-negative");
			}

		[UnityTest]
		public IEnumerator DungeonDelveMode_CanAbortDungeon()
			{
			// Arrange
			dungeonMode.StartDungeonDelve();
			yield return new UnityEngine.WaitForSeconds(0.1f);
		...
		[UnityTest]
		public IEnumerator DungeonDelveMode_CanResetForNewSession()
			{
			// Arrange
			dungeonMode.StartDungeonDelve();
			yield return new UnityEngine.WaitForSeconds(0.1f);
			...
			// Assert
			Assert.AreEqual(DungeonDelveState.NotStarted, dungeonMode.CurrentState,
				"State should be NotStarted after reset");
			}
		...
		public IEnumerator DungeonDelveMode_GeneratesThreeFloors()
			...
			// Assert
			Assert.AreEqual(DungeonDelveState.InProgress, dungeonMode.CurrentState,
				"Dungeon should be in progress after generation");
		...
		[UnityTest]
		public IEnumerator DungeonDelveMode_GeneratesUniqueFloorSeeds()
			{
			// Arrange
			...
			foreach (var testSeed in testSeeds)
				...
				testDungeonMode.SetEntityManagerForTesting(entityManager);
				...
				Assert.AreEqual(floorSeeds.Count, uniqueSeeds.Count,
					$"All floor seeds should be unique for dungeon seed {testSeed}");
			...
			for (int i = 0; i < generatedSeedSets.Count; i++)
				...
				for (int j = i + 1; j < generatedSeedSets.Count; j++)
					...
					Assert.IsFalse(seedSetsAreIdentical,
						$"Different dungeon seeds should produce different floor seed sets");
					}
				}
			}
		...
		public IEnumerator DungeonDelveMode_PlacesCorrectNumberOfBosses()
			...
			Assert.IsTrue(bossHealthValues[2] > bossHealthValues[0],
				"Final boss should be stronger than first mini-boss");
			Assert.IsTrue(bossHealthValues[2] > bossHealthValues[1],
				"Final boss should be stronger than second mini-boss");
			}
		...
		public IEnumerator DungeonDelveMode_PlacesThreeProgressionLocks()
			...
			// Assert
			Assert.AreEqual(expectedLocks, dungeonMode.ActiveProgressionLocks.Count,
				"Should place exactly 3 progression locks");
		...
		public IEnumerator DungeonDelveMode_PlacesMinimumSecretsPerFloor()
			...
			// Assert
			Assert.IsTrue(dungeonMode.ActiveSecrets.Count >= minimumSecretsTotal,
				"Should place at least 1 secret per floor");
			Assert.AreEqual(expectedSecrets, dungeonMode.ActiveSecrets.Count,
				"Should place exactly 6 secrets total (1+2+3)");
			...
			for (int i = 0; i < dungeonMode.ActiveSecrets.Count; i++)
				...
				Assert.IsTrue(secretComponent.FloorIndex >= 0 && secretComponent.FloorIndex < 3,
					$"Secret {i} should have valid floor index");
				}
		...
		public IEnumerator DungeonDelveMode_PlacesAllPickupTypes()
			...
			foreach (PickupType expectedType in allPickupTypes)
				{
				Assert.IsTrue(placedPickupTypes.Contains(expectedType),
					$"Pickup type {expectedType} should be placed in dungeon");
				}
