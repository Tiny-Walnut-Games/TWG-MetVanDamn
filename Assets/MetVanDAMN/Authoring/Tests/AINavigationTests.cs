...
#nullable enable
	public class AINavigationTests
		...
		public void NavigationGraphBuildSystem_Creation_DoesNotThrow()
			...
			Assert.DoesNotThrow(() =>
			{
				SystemHandle system = this._testWorld.GetOrCreateSystem<NavigationGraphBuildSystem>();
		...
		public void AINavigationSystem_Creation_DoesNotThrow()
			...
			Assert.DoesNotThrow(() =>
			{
				SystemHandle system = this._testWorld.GetOrCreateSystem<AINavigationSystem>();
				Assert.IsNotNull(system);
			});
			}
		...
		public void NavigationValidationSystem_Creation_DoesNotThrow()
			...
			Assert.DoesNotThrow(() =>
			{
				SystemHandle system = this._testWorld.GetOrCreateSystem<NavigationValidationSystem>();
				Assert.IsNotNull(system);
			});
			}
		...
		public void NavigationValidationUtility_GenerateValidationReport_DoesNotThrow()
			...
			Assert.DoesNotThrow(() =>
			{
				NavigationValidationReport report = NavigationValidationUtility.GenerateValidationReport(this._testWorld);
				report.Dispose();
			});
			}
		...
		public void NavigationIntegrationTest_FullScenario_WorksCorrectly()
			...
			this._entityManager.AddComponentData(node1Entity, new NavNode(1, new float3(0, 0, 0), BiomeType.SolarPlains, Polarity.Sun));
			this._entityManager.AddComponentData(node1Entity, new NodeId { _value = 1 });
			this._entityManager.AddBuffer<Core.NavLinkBufferElement>(node1Entity);

			Entity node2Entity = this._entityManager.CreateEntity();
			this._entityManager.AddComponentData(node2Entity, new NavNode(2, new float3(10, 0, 0), BiomeType.ShadowRealms, Polarity.Moon));
			this._entityManager.AddComponentData(node2Entity, new NodeId { _value = 2 });
			this._entityManager.AddBuffer<NavLinkBufferElement>(node2Entity);

			// Create navigation link
			DynamicBuffer<NavLinkBufferElement> linkBuffer = this._entityManager.GetBuffer<NavLinkBufferElement>(node1Entity);
			var navLink = new NavLink(1, 2, ConnectionType.Bidirectional, Polarity.Sun, Ability.None);
			linkBuffer.Add(navLink);

			// Create agent entity
			Entity agentEntity = this._entityManager.CreateEntity();
			this._entityManager.AddComponentData(agentEntity, new AINavigationState(1, 2));
			this._entityManager.AddComponentData(agentEntity, new AgentCapabilities(Polarity.Sun, Ability.Jump, 0.8f));
			this._entityManager.AddBuffer<PathNodeBufferElement>(agentEntity);

			// Create navigation graph singleton
			Entity navGraphEntity = this._entityManager.CreateEntity();
			this._entityManager.AddComponentData(navGraphEntity, new NavigationGraph(2, 1) { IsReady = true });

			// Act: Test agent can navigate
			AgentCapabilities agent = this._entityManager.GetComponentData<AgentCapabilities>(agentEntity);
			...
			Assert.AreEqual(2, this._entityManager.GetComponentData<NavigationGraph>(navGraphEntity).NodeCount);
			}
