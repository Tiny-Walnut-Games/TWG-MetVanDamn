// --------------------------------------------------------------------------------------------------
// Legacy Snapshot: Original WorldBootstrapTests (preserved non-destructively)
// --------------------------------------------------------------------------------------------------
// This file captures the user's original test implementation prior to refactors that introduced the
// grouped *GenerationSettings structs. The types referenced below (WorldSettings, BiomeSettings,
// DistrictSettings, DebugSettings, etc.) no longer exist in the current API surface. To avoid
// compilation errors while still honoring the original work, the entire block is wrapped in a
// conditional compilation symbol.
//
// To review or port the legacy logic, define LEGACY_WORLD_BOOTSTRAP in your test assembly build
// settings and then iteratively adapt the code to current data structures.
// --------------------------------------------------------------------------------------------------
// NOTE: Intentionally left exactly as provided (including any now-invalid identifiers) to preserve
// original authorial intent. Do not "optimize away" without explicit direction.
// --------------------------------------------------------------------------------------------------

#if LEGACY_WORLD_BOOTSTRAP
...
namespace TinyWalnutGames.MetVD.Authoring.Tests
{
    ...
    /// </summary>
    public class WorldBootstrapTests_Legacy
    {
