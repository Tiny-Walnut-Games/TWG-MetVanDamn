...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DemoInventoryUI : MonoBehaviour
		{
		[Header("UI Panels")] public GameObject? inventoryPanel;

		public Transform? inventoryGrid;
		public Transform? equipmentPanel;

		[Header("Equipment Slots")] public DemoEquipmentSlot? weaponSlot;

		public DemoEquipmentSlot? offhandSlot;
		public DemoEquipmentSlot? armorSlot;
		public DemoEquipmentSlot? trinketSlot;

		[Header("Item Display")] public GameObject? itemSlotPrefab;

		public GameObject? tooltipPanel;
		public Text? tooltipText;

		[Header("Player Info")] public Slider? healthBar;
		...
		private void CreateEquipmentPanel()
			...
			RectTransform rectTransform = equipObj.GetComponent<RectTransform>();
		...
		private void CreateEquipmentSlot(string slotName, EquipmentSlot slotType, Vector2 anchorPosition)
			...
			DemoEquipmentSlot equipSlot = slotObj.AddComponent<DemoEquipmentSlot>();
		...
		private void CreatePlayerInfoPanel()
			...
			RectTransform rectTransform = infoObj.GetComponent<RectTransform>();
		...
		private void InitializeInventorySlots()
			...
			foreach (DemoInventorySlot slot in inventorySlots)
				{
			...
			for (int i = 0; i < 20; i++) // Default inventory size
				...
				DemoInventorySlot inventorySlot = slotObj.AddComponent<DemoInventorySlot>();
		...
		private void RefreshInventoryDisplay()
			...
			List<DemoItem> items = playerInventory.GetInventoryItems();
		...
		private void UpdateCoinsDisplay()
			{
			if (playerInventory == null || coinsText == null) return;
		...
		private void OnItemEquipped(DemoItem item)
			...
			DemoEquipmentSlot? slot = GetEquipmentSlotForItem(item);
			if (slot)
				{
				slot.SetItem(item);
				}
	...
	public class DemoInventorySlot : MonoBehaviour
		...
		private Image slotImage = null!; // set in Initialize
		private int slotIndex;
		private Text stackText = null!; // set in Initialize
		...
		private void OnSlotClicked()
			...
			if (currentItem!.type == ItemType.Consumable)
				{
				// Use consumable
				if (inventoryUI!.PlayerInventory != null) inventoryUI.PlayerInventory.UseItem(currentItem!);
				}
				...
				{
				// Equip item
				if (inventoryUI!.PlayerInventory != null) inventoryUI.PlayerInventory.EquipItem(currentItem!);
				}
			}
		}
	...
	public class DemoEquipmentSlot : MonoBehaviour
		...
		private void OnSlotClicked()
			{
			if (equippedItem != null && inventoryUI != null && inventoryUI.PlayerInventory != null)
				{
				// Unequip item
