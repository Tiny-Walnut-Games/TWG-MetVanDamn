...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class UpgradeEffectApplicator : MonoBehaviour
		...
		private PlayerStats baseStats = new();

		[SerializeField] private PlayerStats currentStats = new();

		[Header("Modifiers")] [SerializeField] private List<StatModifier> activeModifiers = new();
		...
		// Read-only exposure for editor / debugging without granting mutation rights
		...
		public void ApplyUpgrade(UpgradeDefinition upgrade)
			{
			if (upgrade == null)
				return; // Defensive: external callers may still pass null pre-migration

			// Sentinel upgrades represent deliberate "no selection" and must be inert
			if (UpgradeCollection.IsSentinel(upgrade))
				{
				if (enableDebugLogging)
					{
					Debug.Log(
						"⚪ Sentinel upgrade received – no-op applied (deterministic non-null invariant maintained).");
					}
			...
			foreach (string effectId in upgrade.CustomEffectIds)
				{
		...
		private void RecalculateStats()
			...
			if (enableDebugLogging)
				{
				Debug.Log(
					$"📊 Stats recalculated: Health {currentStats.maxHealth}, Speed {currentStats.runSpeed}, Damage {currentStats.attackDamage}");
				}
			}
			...
			switch (modifier.statName.ToLower())
				{
					...
					case "criticalchance":
					case "crit":
						currentStats.criticalChance =
							Mathf.Clamp01(ApplyModifierValue(currentStats.criticalChance, modifier));
						break;
					case "damagereduction":
					case "armor":
						currentStats.damageReduction =
							Mathf.Clamp01(ApplyModifierValue(currentStats.damageReduction, modifier));
						break;
					...
					case "scanrange":
						...
						break;
					case "inventoryslots":
						currentStats.inventorySlots =
							Mathf.RoundToInt(ApplyModifierValue(currentStats.inventorySlots, modifier));
						break;
			...
			switch (effectId.ToLower())
				{
					...
					case "mapreveal":
						...
						if (playerInventory != null)
							...
							float newScanRange =
								currentScanRange + upgrade.Value * 10f; // Multiply effect for map reveal

							// Apply the enhanced scan range
							var modifier = new StatModifier(upgrade.Id, "scanrange", ModifierType.Additive,
								upgrade.Value * 10f);
							ApplyStatModifier(modifier);
		...
		private void LogCurrentStats()
			...
			Debug.Log(
				$"Movement: Walk {currentStats.walkSpeed}, Run {currentStats.runSpeed}, Jump {currentStats.jumpForce}");
			Debug.Log(
				$"Combat: Health {currentStats.maxHealth}, Damage {currentStats.attackDamage}, Speed {currentStats.attackSpeed}");
			Debug.Log(
				$"Utility: Interaction {currentStats.interactionRange}, Scan {currentStats.scanRange}, Inventory {currentStats.inventorySlots}");
		...
		// ---------------------------------------------------------------------
		// Context menu utilities to quickly inspect modifier + stat state
		// without needing a custom inspector yet. Safe in builds (stripped)
		// but intended for rapid iteration during balancing.
		// =====================================================================
		[ContextMenu("UpgradeEffect/Dump Active Modifiers")]
		private void DumpActiveModifiers()
			{
			if (activeModifiers.Count == 0)
				{
				Debug.Log("[UpgradeEffectApplicator] No active modifiers.");
				return;
				}

			Debug.Log($"[UpgradeEffectApplicator] Active Modifiers ({activeModifiers.Count}):");
			for (int i = 0; i < activeModifiers.Count; i++)
				{
				StatModifier m = activeModifiers[i];
				Debug.Log(
					$" #{i + 1}: upgradeId={m.upgradeId}, stat={m.statName}, type={m.modifierType}, value={m.value}, active={m.isActive}");
				}
			}

		[ContextMenu("UpgradeEffect/Dump Stats + Modifiers")]
		private void DumpStatsAndModifiers()
			{
			LogCurrentStats();
			DumpActiveModifiers();
			}
		...
		private System.Collections.IEnumerator MapRevealCoroutine(float revealRange)
			...
			while (true)
				...
				Collider[] nearbyObjects = Physics.OverlapSphere(transform.position, revealRange);

				foreach (Collider? obj in nearbyObjects)
					{
					// Mark treasures, enemies, and interactive elements as revealed
					if (obj.TryGetComponent<DemoTreasureChest>(out DemoTreasureChest? treasure))
						{
						// Add visual indicator or glow effect for revealed treasures
						AddRevealEffect(obj.gameObject, Color.yellow);
						}

					if (obj.TryGetComponent<DemoEnemyAI>(out DemoEnemyAI? enemy))
						{
						// Add visual indicator for revealed enemies
						AddRevealEffect(obj.gameObject, Color.red);
						}

					if (obj.TryGetComponent<IDemoInteractable>(out IDemoInteractable? interactable))
						{
		...
		private void AddRevealEffect(GameObject target, Color effectColor)
			...
			MapRevealEffect revealEffect = target.AddComponent<MapRevealEffect>();
		...
		public class PlayerStats
			...
			[Header("Combat")] public int maxHealth = 100;
			...
			[Header("Utility")] public float interactionRange = 2f;
		...
		private class MapRevealEffect : MonoBehaviour
			...
			private Light revealLight = null!; // created during Initialize; null-forgiving to satisfy analyzer
