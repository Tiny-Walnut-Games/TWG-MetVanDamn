...
namespace TinyWalnutGames.MetVanDAMN.Authoring
{
    ...
    public class DemoBossAI : MonoBehaviour, IDemoDamageable
    {
        ...
        public BossPhase CurrentPhase => currentPhaseIndex < phases.Length ? phases[currentPhaseIndex] : null;
        ...
        private void MoveTowardsPlayer()
        {
            ...
            {
                rb3D.velocity = direction * currentSpeed;
            }
            else
            {
        ...
        {
            float damage = baseDamage * (CurrentPhase?.damageMultiplier ?? 1f);
        ...
        private IEnumerator ExecuteBasicRush()
        {
            ...
            while (elapsed < rushDuration)
            {
                if (rb2D)
                {
                    rb2D.velocity = rushDirection * rushSpeed;
                }
                else if (rb3D)
                {
                    rb3D.velocity = rushDirection * rushSpeed;
                }
        ...
        private IEnumerator ExecuteTelegraphedSlam()
        {
            ...
            // Execute slam attack
            float slamRadius = 4f;
            float damage = baseDamage * 2f * (CurrentPhase?.damageMultiplier ?? 1f);
            
            if (Vector3.Distance(target.position, slamPosition) <= slamRadius)
            {
                var playerCombat = target.GetComponent<DemoPlayerCombat>();
                if (playerCombat)
                {
        ...
        private IEnumerator ExecuteSummonMinions()
        {
            ...
            for (int i = 0; i < minionsToSummon; i++)
            {
                ...
                var ai = minion.GetComponent<DemoEnemyAI>();
                if (!ai)
                {
        ...
        private IEnumerator ExecuteArenaHazard()
        {
            ...
            foreach (Vector3 pos in hazardPositions)
            {
                ...
                float damage = baseDamage * 1.5f * (CurrentPhase?.damageMultiplier ?? 1f);
                
                if (Vector3.Distance(target.position, pos) <= hazardRadius)
                {
                    var playerCombat = target.GetComponent<DemoPlayerCombat>();
                    if (playerCombat)
                    {
