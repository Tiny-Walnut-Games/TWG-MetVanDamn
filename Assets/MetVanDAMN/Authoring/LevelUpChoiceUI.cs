// Phase 2 Nullability refinement for LevelUpChoiceUI
...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class LevelUpChoiceUI : MonoBehaviour
		...
		[Header("Choice Button Settings")] [SerializeField]
		private float buttonSpacing = 20f;

		[SerializeField] private Vector2 buttonSize = new(300f, 120f);

		[Header("Audio")] [SerializeField] private AudioClip? choiceConfirmSound = null;

		[SerializeField] private AudioClip? choiceHoverSound = null;
		[SerializeField] private AudioClip? panelOpenSound = null;

		[Header("Debug")] [SerializeField] private bool enableDebugLogging = true;

		private AudioSource? audioSource = null;
		private List<LevelUpChoiceButton> choiceButtons = new();
		private LevelUpChoiceSystem? choiceSystem = null;

		// Runtime data
		private UpgradeDefinition[]? currentChoices = null;
		...
		private void Awake()
			...
			// Find choice system
			choiceSystem = FindFirstObjectByType<LevelUpChoiceSystem>();
		...
		private void OnDestroy()
			{
			// Unsubscribe if the system still exists; Unity will clear delegates on domain reload
			if (choiceSystem != null)
				{
				choiceSystem.OnChoicesGenerated -= ShowChoices;
				}
		...
		private void CreateTitleText()
			...
			if (choicePanel == null)
				{
				return; // safety guard; construction should have created panel
				}
		...
		private void CreateCancelButton()
			...
			Image buttonImage = cancelObj.AddComponent<Image>();
		...
		private void ClearChoiceButtons()
			{
			foreach (LevelUpChoiceButton button in choiceButtons)
				{
				if (button != null)
					{
		...
		private void CreateChoiceButtons()
			...
			if (choiceContainer == null)
				{
				if (enableDebugLogging) Debug.LogWarning("Choice container missing when creating buttons");
				return;
				}

			for (int i = 0; i < currentChoices.Length; i++)
				{
				UpgradeDefinition choice = currentChoices[i];
				LevelUpChoiceButton buttonObj = CreateChoiceButton(choice, i);
		...
		private LevelUpChoiceButton CreateChoiceButton(UpgradeDefinition upgrade, int index)
			...
			LevelUpChoiceButton choiceButton = buttonObj.AddComponent<LevelUpChoiceButton>();
		...
		/// </summary>
		private void SetUpgradeData(UpgradeDefinition upgrade, string name, string description,
			UpgradeCategory category)
			{
			// Use reflection to set private serialized fields for testing
			FieldInfo nameField = typeof(UpgradeDefinition).GetField("upgradeName",
				System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
			FieldInfo descField = typeof(UpgradeDefinition).GetField("description",
				System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
			FieldInfo categoryField = typeof(UpgradeDefinition).GetField("category",
				System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
			FieldInfo valueField = typeof(UpgradeDefinition).GetField("value",
				System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
			FieldInfo modifierTypeField = typeof(UpgradeDefinition).GetField("modifierType",
				System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
			FieldInfo targetStatField = typeof(UpgradeDefinition).GetField("targetStat",
				System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
