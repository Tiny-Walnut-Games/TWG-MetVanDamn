...
namespace TinyWalnutGames.MetVD.Authoring
	...
	public partial class BiomeTransitionMainThreadSystem : SystemBase
		...
		protected override void OnUpdate()
			...
			var candidates =
				new System.Collections.Generic.List<(Entity entity, BiomeArtProfile profile, BiomeTransition transition,
					NodeId nodeId)>();
			...
			Entities
				...
				.ForEach((Entity entity, ref BiomeTransition transition,
					...
					bool hasExplicit = artProfile.transitionFromTile != null || artProfile.transitionBlendA != null ||
					                   artProfile.transitionBlendB != null || artProfile.transitionToTile != null;
		...
		private BiomeArtProfile? GetArtProfileForBiome(BiomeType biomeType)
			...
			foreach (string g in guids)
				...
				if (profile != null && string.Equals(profile.biomeName, biomeType.ToString(),
		...
		private void ApplyTransitionTiles(BiomeArtProfile artProfile, BiomeTransition transition, NodeId nodeId)
			{
			if (artProfile == null) return; // safety guard
			BiomeArtProfile profile = artProfile; // local non-null alias for analyzers
			...
			TileBase[] tiles = artProfile.transitionTiles ?? Array.Empty<TileBase>();
			...
			for (int ox = -radius; ox <= radius; ox++)
				...
				for (int oy = -radius; oy <= radius; oy++)
					...
					float deadzone = math.clamp(profile.transitionDeadzone, 0f, 0.5f);
					...
					Color colorB = otherProfile != null
						? otherProfile.debugColor
						: GenerateHashBasedBiomeColor(transition.ToBiome);
			...
			Debug.Log(
				$"Applied blended transition tiles for {transition.FromBiome}->{transition.ToBiome} at node {nodeId.Value} (r={radius})");
			}
		}
	}
