...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class PlayerLevelProgression : MonoBehaviour
		...
		[Header("Abilities")] [SerializeField] private Ability startingAbilities = Ability.Jump;

		[SerializeField] private Ability currentAbilities = Ability.Jump;

		[Header("Applied Upgrades")] [SerializeField]
		private List<string> appliedUpgradeIds = new();

		[SerializeField] private List<AppliedUpgrade> appliedUpgrades = new();

		[Header("Save/Load")] [SerializeField] private bool autoSave = true;
		...
		private void Start()
			...
			if (enableDebugLogging)
				{
				Debug.Log(
					$"🎮 Player progression initialized: Level {currentLevel}, XP {currentXP}/{XPRequiredForNextLevel}, Abilities: {currentAbilities}");
				}
			}
		...
		private void ReapplyAllUpgrades()
			...
			UpgradeDatabaseManager upgradeDatabase = FindFirstObjectByType<UpgradeDatabaseManager>();
			if (upgradeDatabase != null)
				{
				foreach (string upgradeId in appliedUpgradeIds)
					{
					UpgradeDefinition upgrade = upgradeDatabase.GetUpgradeById(upgradeId);
					if (upgrade != null)
						{
						currentAbilities |= upgrade.GrantsAbilities;
		...
		private class ProgressionSaveData
			...
			public string[] upgradeIds = System.Array.Empty<string>();
		...
		private void LoadProgression()
			...
			if (PlayerPrefs.HasKey(saveKey))
				...
				try
					...
					ProgressionSaveData saveData = JsonUtility.FromJson<ProgressionSaveData>(json);
