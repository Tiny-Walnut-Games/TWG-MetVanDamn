#nullable enable
// Phase 2 Nullability Pass: DungeonDelveMainMenu
//  - Enabled nullable reference types (#nullable enable)
//  - Serialized Unity object references annotated nullable (assigned via Inspector or runtime factory)
//  - Added explicit null initializers to satisfy analyzers without altering runtime behavior
//  - Non-invasive: no logic changes, only annotations & comments
#nullable enable
using UnityEngine;
...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DungeonDelveMainMenu : MonoBehaviour
		...
		[SerializeField] private Button? startDungeonButton = null;
		[SerializeField] private Button? configureButton = null;
		[SerializeField] private Button? exitButton = null;
		[SerializeField] private Text? titleText = null;
		[SerializeField] private Text? statusText = null;
		[SerializeField] private Text? seedDisplayText = null;

		[Header("Configuration Panel")] [SerializeField]
		private GameObject? configurationPanel = null;

		[SerializeField] private InputField? seedInputField = null;
		[SerializeField] private Toggle? customSeedToggle = null;
		[SerializeField] private Button? generateRandomSeedButton = null;
		[SerializeField] private Button? applyConfigButton = null;
		[SerializeField] private Button? cancelConfigButton = null;

		[Header("HUD Elements")] [SerializeField]
		private GameObject? dungeonHUD = null;

		[SerializeField] private Text? floorText = null;
		[SerializeField] private Text? timeText = null;
		[SerializeField] private Text? bossesDefeatedText = null;
		[SerializeField] private Text? secretsFoundText = null;
		[SerializeField] private Text? progressionLocksText = null;
		[SerializeField] private Button? abortDungeonButton = null;

		[Header("Completion Panel")] [SerializeField]
		private GameObject? completionPanel = null;

		[SerializeField] private Text? completionTimeText = null;
		[SerializeField] private Text? completionStatsText = null;
		[SerializeField] private Button? returnToMenuButton = null;
		[SerializeField] private Button? startNewDungeonButton = null;
		private uint configuredSeed = 42;

		// State
		private DungeonDelveMode? currentDungeonMode = null; // Created or discovered at runtime
		...
		private void OnDestroy()
			...
			if (currentDungeonMode)
				{
				DungeonDelveMode mode = currentDungeonMode;
				if (mode != null)
					{
					mode.OnSessionAborted -= OnSessionAborted;
					}
				}
			}
		...
		private void CreateMainMenuElements()
			...
			if (mainMenuCanvas == null)
				{
				Debug.LogWarning(
					"DungeonDelveMainMenu: mainMenuCanvas unexpectedly null while creating title; creating fallback canvas.");
				Canvas fallback = new GameObject("FallbackCanvas").AddComponent<Canvas>();
				fallback.renderMode = RenderMode.ScreenSpaceOverlay;
				mainMenuCanvas = fallback; // assign for rest of lifecycle
				}
			...
			if (mainMenuCanvas == null)
				{
				Debug.LogWarning(
					"DungeonDelveMainMenu: mainMenuCanvas unexpectedly null while creating seed display; creating fallback canvas.");
				Canvas fallback = new GameObject("FallbackCanvas").AddComponent<Canvas>();
				fallback.renderMode = RenderMode.ScreenSpaceOverlay;
				mainMenuCanvas = fallback; // assign for rest of lifecycle
				}
			...
			RectTransform seedRect = seedDisplayText.GetComponent<RectTransform>();
			...
			// Ensure seed display updated through helper (null-safe)
			UpdateSeedDisplay();
			}
		...
		private void CreateProgressDisplays(GameObject parent)
			...
			RectTransform bossesRect = bossesDefeatedText.GetComponent<RectTransform>();
			...
			RectTransform secretsRect = secretsFoundText.GetComponent<RectTransform>();
			...
			RectTransform locksRect = progressionLocksText.GetComponent<RectTransform>();
		...
		private void CreateCompletionPanel()
			...
			RectTransform completionRect = completionTimeText.GetComponent<RectTransform>();
			...
			RectTransform statsRect = completionStatsText.GetComponent<RectTransform>();
		...
		private void FindOrCreateDungeonMode()
			{
			currentDungeonMode = FindFirstObjectByType<DungeonDelveMode>();
			if (!currentDungeonMode)
				{
		...
		private void CreateConfigurationElements(GameObject parent)
			...
			RectTransform toggleRect = customSeedToggle.GetComponent<RectTransform>();
			...
			RectTransform inputRect = seedInputField.GetComponent<RectTransform>();
		...
		private void UpdateDungeonHUD()
			...
			if (bossesDefeatedText)
				...
				if (currentDungeonMode != null)
					...
					foreach (GameObject boss in currentDungeonMode.ActiveBosses)
						{
						if (boss != null)
							{
							DemoBossAI bossAI = boss.GetComponent<DemoBossAI>();
							if (bossAI != null && bossAI.GetCurrentHealth() <= 0)
								{
								defeated++;
								}
							}
			...
			if (secretsFoundText)
				{
				// currentDungeonMode asserted non-null earlier in method
			...
			if (progressionLocksText)
				...
				if (currentDungeonMode != null)
					{
					foreach (GameObject lockObj in currentDungeonMode.ActiveProgressionLocks)
						{
						if (lockObj != null)
							{
							DungeonProgressionLock lockComponent = lockObj.GetComponent<DungeonProgressionLock>();
		...
		private void ExitToMainMenu()
			...
			DungeonDelveSceneManager sceneManager = FindFirstObjectByType<DungeonDelveSceneManager>();
		...
		// (Removed duplicate recursive UpdateSeedDisplay definition near end of file)
	...
	public class DungeonDelveSceneManager : MonoBehaviour
		...
		private string dungeonDelveSceneName = "DungeonDelveMode";
