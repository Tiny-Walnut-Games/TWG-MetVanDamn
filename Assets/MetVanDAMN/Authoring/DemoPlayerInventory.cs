...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DemoPlayerInventory : MonoBehaviour
		{
		[Header("Inventory Settings")] public int inventorySize = 20;

		[Header("Input")] public KeyCode inventoryKey = KeyCode.I;
		...
		public DemoItem? EquippedWeapon
			{
			get { return equippedWeapon; }
			}

		public DemoItem? EquippedOffhand
			{
			get { return equippedOffhand; }
			}

		public DemoItem? EquippedArmor
			{
			get { return equippedArmor; }
			}

		public DemoItem? EquippedTrinket
			{
		...
		public bool AddItem(DemoItem item, int quantity = 1)
			...
			if (item.stackSize > 1)
				{
				foreach (DemoItem existingItem in inventoryItems)
					{
		...
		public bool HasItem(string itemId, int quantity = 1)
			{
			int totalCount = 0;
			foreach (DemoItem item in inventoryItems)
				{
				if (item.id == itemId)
					{
		...
		private void ApplySpeedBuff(DemoItem item)
			...
			DemoPlayerMovement playerMovement = GetComponent<DemoPlayerMovement>();
			if (playerMovement)
				{
		...
		private System.Collections.IEnumerator ApplyTemporarySpeedBuff(DemoPlayerMovement movement,
		...
		private void ToggleInventoryUI()
			...
			DemoInventoryUI inventoryUI = FindFirstObjectByType<DemoInventoryUI>();
			if (inventoryUI)
				{
		...
		private void CreateInventoryUI()
			...
			DemoInventoryUI inventoryUI = inventoryUIObj.AddComponent<DemoInventoryUI>();
		...
		/// </summary>
		public void SetStats(int newInventorySlots, float newInteractionRange, float newScanRange,
			float newLootMagnetRange)
			{
		...
		private System.Collections.IEnumerator AutoLootCoroutine()
			...
			while (autoLootEnabled)
				...
				Collider[] nearbyItems = Physics.OverlapSphere(transform.position, lootMagnetRange);

				foreach (Collider? item in nearbyItems)
					{
					DemoLootDrop lootComponent = item.GetComponent<DemoLootDrop>();
					if (lootComponent && !IsInventoryFull())
						{
						DemoItem lootItem = lootComponent.GetItem();
						if (lootItem != null)
							{
							AddItem(lootItem);
