...
#pragma warning disable IDE0090 // 'new' expression can be simplified (explicit types kept for clarity and Unity version compatibility)
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DungeonDelveMode : MonoBehaviour
		{
		[Header("Dungeon Configuration")] [SerializeField]
		private uint dungeonSeed = 42;
		...
		[Header("Boss Configuration")] [SerializeField]
		private string[] miniBossNames = { "Crystal Guardian", "Magma Serpent" };
		...
		[Header("Progression System")] [SerializeField]
		private string[] progressionLockNames = { "Crystal Key", "Flame Essence", "Void Core" };

		[SerializeField] private Color[] lockColors = { Color.cyan, Color.red, new Color(0.5f, 0.1f, 0.9f) };

		[Header("Rewards & Pickups")] [SerializeField]
		private int baseHealthPickups = 3;
		...
		private DungeonDelveState currentState = DungeonDelveState.NotStarted;
		...
		// Events (nullable – subscribers optional)
		...
		/// <summary>
		/// Sets the EntityManager for testing purposes. Only use in test environments.
		/// </summary>
		public void SetEntityManagerForTesting(EntityManager testEntityManager)
			{
			entityManager = testEntityManager;
			defaultWorld = testEntityManager.World;
			}
		...
		private void InitializeComponents()
			...
			playerMovement = FindFirstObjectByType<DemoPlayerMovement>();
			playerCombat = FindFirstObjectByType<DemoPlayerCombat>();
			playerInventory = FindFirstObjectByType<DemoPlayerInventory>();

			// Find managers
			aiManager = FindFirstObjectByType<DemoAIManager>();
			lootManager = FindFirstObjectByType<DemoLootManager>();
			mapGenerator = FindFirstObjectByType<MetVanDAMNMapGenerator>();
		...
		private void ClearDungeonObjects()
			...
			foreach (GameObject lockObj in activeProgressionLocks)
				{
			...
			foreach (GameObject secret in activeSecrets)
				{
		...
		private IEnumerator GenerateFloorLayout(DungeonFloor floor)
			...
			Entity worldConfigEntity = entityManager.CreateEntity();
			entityManager.AddComponentData(worldConfigEntity, new WorldSeed { Value = floor.seed });
		...
		private void CreateDistrictsForFloor(DungeonFloor floor)
			...
			entityManager.AddComponentData(hubEntity,
				new NodeId { _value = (uint)(floor.floorIndex * 1000), Coordinates = int2.zero });
			entityManager.AddComponentData(hubEntity, new WfcState());

			// Add district-specific components
			if (!entityManager.HasBuffer<WfcCandidateBufferElement>(hubEntity))
				entityManager.AddBuffer<WfcCandidateBufferElement>(hubEntity);
			if (!entityManager.HasBuffer<ConnectionBufferElement>(hubEntity))
				entityManager.AddBuffer<ConnectionBufferElement>(hubEntity);
			if (!entityManager.HasBuffer<GateConditionBufferElement>(hubEntity))
				entityManager.AddBuffer<GateConditionBufferElement>(hubEntity);
			...
			for (int i = 0; i < floor.roomCount - 1; i++)
				...
				// Add buffers
				if (!entityManager.HasBuffer<WfcCandidateBufferElement>(districtEntity))
					entityManager.AddBuffer<WfcCandidateBufferElement>(districtEntity);
				if (!entityManager.HasBuffer<ConnectionBufferElement>(districtEntity))
					entityManager.AddBuffer<ConnectionBufferElement>(districtEntity);
				if (!entityManager.HasBuffer<GateConditionBufferElement>(districtEntity))
		...
		private void PlaceMiniBoss(int floorIndex, Vector3 position)
			...
			DemoBossAI bossAI = bossGO.AddComponent<DemoBossAI>();
			// Avoid null-propagation on UnityEngine.Object; pass Transform only if available
			Transform? playerTransform = playerMovement ? playerMovement.transform : null;
			// Provide a safe default if API expects non-null: DemoBossAI can internally handle null or we pass bossGO.transform
			bossAI.Initialize(playerTransform ?? bossGO.transform, aiManager);
		...
		private void ApplyBiomeThemingToBoss(DemoBossAI bossAI, int floorIndex)
			{
			Renderer renderer = bossAI.GetComponent<Renderer>();
			if (!renderer)
				{
				renderer = bossAI.gameObject.AddComponent<MeshRenderer>();
				MeshFilter meshFilter = bossAI.gameObject.AddComponent<MeshFilter>();
		...
		private void PlaceProgressionLock(int floorIndex, Vector3 position)
			...
			MeshRenderer renderer = lockGO.AddComponent<MeshRenderer>();
			MeshFilter meshFilter = lockGO.AddComponent<MeshFilter>();
			meshFilter.mesh = CreateLockMesh();
			renderer.material = new Material(Shader.Find("Universal Render Pipeline/Lit"));
			renderer.material.color = lockColors[floorIndex];

			// Add interaction component
			DungeonProgressionLock lockInteraction = lockGO.AddComponent<DungeonProgressionLock>();
		...
		private void PlaceSecret(int floorIndex, int secretIndex, Vector3 position)
			...
			MeshRenderer renderer = secretGO.AddComponent<MeshRenderer>();
			MeshFilter meshFilter = secretGO.AddComponent<MeshFilter>();
			meshFilter.mesh = CreateSecretMesh();
			renderer.material = new Material(Shader.Find("Universal Render Pipeline/Lit"));
			renderer.material.color = biomeColors[floorIndex] * 0.7f; // Slightly darker than biome color

			// Add interaction component
			DungeonSecret secretInteraction = secretGO.AddComponent<DungeonSecret>();
		...
		private Mesh CreateSecretMesh()
			...
			vertices.AddRange(new Vector3[]
				{
				new Vector3(-1, t, 0) * scale, new Vector3(1, t, 0) * scale, new Vector3(-1, -t, 0) * scale,
				new Vector3(1, -t, 0) * scale,
				new Vector3(0, -1, t) * scale, new Vector3(0, 1, t) * scale, new Vector3(0, -1, -t) * scale,
				new Vector3(0, 1, -t) * scale,
				new Vector3(t, 0, -1) * scale, new Vector3(t, 0, 1) * scale, new Vector3(-t, 0, -1) * scale,
		...
		private void PlacePickup(PickupType type, Vector3 position, int floorIndex)
			...
			// Add visual representation
			MeshRenderer renderer = pickupGO.AddComponent<MeshRenderer>();
			MeshFilter meshFilter = pickupGO.AddComponent<MeshFilter>();
			meshFilter.mesh = CreatePickupMesh(type);
			renderer.material = new Material(Shader.Find("Universal Render Pipeline/Lit"));
			renderer.material.color = GetPickupColor(type);

			// Add interaction component
			DungeonPickup pickupInteraction = pickupGO.AddComponent<DungeonPickup>();
		...
		private Mesh CreateCrossMesh()
			...
			var vertices = new Vector3[]
				{
				// Horizontal bar
				new Vector3(-0.5f, -0.1f, 0), new Vector3(0.5f, -0.1f, 0), new Vector3(0.5f, 0.1f, 0),
				new Vector3(-0.5f, 0.1f, 0),
				// Vertical bar
				new Vector3(-0.1f, -0.5f, 0), new Vector3(0.1f, -0.5f, 0), new Vector3(0.1f, 0.5f, 0),
		...
		private Mesh CreateBottleMesh()
			...
			var vertices = new Vector3[]
				...
				new Vector3(-0.2f, -0.5f, -0.2f), new Vector3(0.2f, -0.5f, -0.2f), new Vector3(0.2f, 0.3f, -0.2f),
				new Vector3(-0.2f, 0.3f, -0.2f),
				new Vector3(-0.2f, -0.5f, 0.2f), new Vector3(0.2f, -0.5f, 0.2f), new Vector3(0.2f, 0.3f, 0.2f),
				new Vector3(-0.2f, 0.3f, 0.2f),
				// Bottle neck
				new Vector3(-0.1f, 0.3f, -0.1f), new Vector3(0.1f, 0.3f, -0.1f), new Vector3(0.1f, 0.5f, -0.1f),
	...
	public class DungeonFloor
		...
		public string? biomeName; // Nullable: assigned during floor generation
