...
namespace TinyWalnutGames.MetVD.Authoring.Editor
	{
#if !METVD_GRAPH_REF
	internal enum DistrictPlacementStrategy : byte
		{
		PoissonDisc = 0,
		JitteredGrid = 1
		}
	...
	public static class ProceduralLayoutGizmoDrawer
		...
		private static void DrawRandomizationModeInfo(WorldConfigurationAuthoring worldConfig)
			...
			Handles.Label(anchor,
				$"Randomization Mode: {worldConfig.randomizationMode}\nSeed: {worldConfig.seed}\nWorld Size: {worldConfig.worldSize}");
			}
		...
		private static void DrawConnectionArrow(Vector3 from, Vector3 to, ConnectionAuthoring connection)
			...
			Color connectionColor = connection.type == ConnectionType.OneWay
		...
		private static void DrawSectorsAndRooms()
			...
			if (_showSectors)
				...
				foreach (int2 c in _sectorCoords)
					...
					Vector3[] quad =
						{
						p - half, p + new Vector3(half.x, 0, -half.z), p + half, p + new Vector3(-half.x, 0, half.z)
						};
		...
		private static void SampleHierarchyEcsData()
			...
			try
				...
				using Unity.Collections.NativeArray<NodeId> nodeIds =
					query.ToComponentDataArray<NodeId>(Unity.Collections.Allocator.Temp);
				_sectorCoords.Clear();
				_roomCoords.Clear();
				...
				{
				/* swallow sampling errors safely */
				}
			}
		...
		private static void PreviewProceduralLayout()
			...
			int targetCount = worldConfig.targetSectors > 0
				? math.min(worldConfig.targetSectors, unplaced.Count)
				: unplaced.Count;
			var random = new Unity.Mathematics.Random((uint)(worldConfig.seed == 0 ? 1 : worldConfig.seed));
			DistrictPlacementStrategy strategy = targetCount > 16
		...
		private static void GeneratePositionsPreview(int2[] positions, int2 worldSize,
			...
			if (strategy == DistrictPlacementStrategy.PoissonDisc)
				...
				for (int i = 0; i < positions.Length; i++)
					...
					float2 jitter = new(random.NextFloat(-jitterAmount, jitterAmount),
						random.NextFloat(-jitterAmount, jitterAmount));
					float2 finalPos = cellCenter + jitter;
					positions[i] = new(math.clamp((int)finalPos.x, 0, worldSize.x - 1),
						math.clamp((int)finalPos.y, 0, worldSize.y - 1));
					}
				}
			}
