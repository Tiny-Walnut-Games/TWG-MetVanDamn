	...
	public class EcsPrefabRegistryGizmoGenerator : UnityEditor.Editor
		...
		internal static void GenerateGizmoPrefabsStatic(EcsPrefabRegistryAuthoring reg)
			...
			for (int i = 0; i < reg.Entries.Count; i++)
				{
				EcsPrefabRegistryAuthoring.Entry entry = reg.Entries [ i ];
				...
				GizmoPrefabMarker marker = go.AddComponent<GizmoPrefabMarker>();
				...
				string path = Path.Combine(reg.GeneratedPrefabsFolder, $"Gizmo_{Sanitize(entry.Key)}.prefab");
				path = path.Replace('\\', '/');
