...
namespace TinyWalnutGames.MetVanDAMN.Authoring.Editor
{
    ...
    {
        [MenuItem("Tools/MetVanDAMN/Compliance Validator", priority = 100)]
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("Ensures unwavering adherence to quality, clarity, and shared mission.", EditorStyles.helpBox);
        ...
        private void OnPlayModeChanged(PlayModeStateChange state)
        {
            if (autoValidateOnPlay && state == PlayModeStateChange.ExitingEditMode)
            {
                if (!RunFullValidation())
                {
                    ...
                    Debug.LogError("❌ Compliance validation failed! Cannot enter play mode until all issues are resolved.");
                }
            }
        }
        ...
        private bool RunFullValidation()
        {
            ...
            {
                Debug.LogError($"❌ Compliance validation failed with {lastReport.failedChecks.Count} issues. See Compliance Validator window for details.");
            }
        ...
        private void ValidateDemoScenes()
        {
            ...
            string[] sceneNames = { "MetVanDAMN_Complete2DPlatformer", "MetVanDAMN_CompleteTopDown", "MetVanDAMN_Complete3D" };
        ...
        private void ValidateNarrativeTest()
        {
            ...
            if (Directory.Exists(docsPath))
            {
                ...
                {
                    lastReport.passedChecks.Add($"✓ Narrative test passed ({narrativePercentage:P0} of docs have narrative coherence)");
                }
                else
                {
                    lastReport.failedChecks.Add($"Narrative test failed ({narrativePercentage:P0} of docs have narrative coherence, need 70%+)");
                }
            }
        ...
        private void ValidateCodeNarrative()
        {
            ...
            float documentationPercentage = sourceFiles.Length > 0 ? (float)wellDocumentedFiles / sourceFiles.Length : 0f;
            
            if (documentationPercentage >= 0.8f) // 80% of code files should be well documented
            {
                lastReport.passedChecks.Add($"✓ Code narrative test passed ({documentationPercentage:P0} of files well documented)");
            }
            else
            {
                lastReport.failedChecks.Add($"Code narrative test failed ({documentationPercentage:P0} of files well documented, need 80%+)");
            }
        }
        ...
        private void DisplayValidationReport()
        {
            ...
            {
                EditorGUILayout.HelpBox($"❌ Compliance validation failed with {lastReport.failedChecks.Count} issues", MessageType.Error);
            }
        ...
        private class ValidationReport
        {
            ...
            public List<string> warningChecks = new List<string>();
    ...
    public class DemoSceneValidator : MonoBehaviour
    {
        ...
        public bool ValidateScene()
        {
            ...
            if (requirePlayerMovement && !FindObjectOfType<DemoPlayerMovement>())
            {
                issues.Add("Missing DemoPlayerMovement component");
                allPassed = false;
            }

            if (requireCombatSystem && !FindObjectOfType<DemoPlayerCombat>())
            {
                issues.Add("Missing DemoPlayerCombat component");
                allPassed = false;
            }

            if (requireInventorySystem && !FindObjectOfType<DemoPlayerInventory>())
            {
            ...
            if (requireLootSystem && !FindObjectOfType<DemoLootManager>())
            {
            ...
            // Check for core MetVanDAMN components
            if (!FindObjectOfType<SmokeTestSceneSetup>())
            {
        ...
        {
            var validator = FindObjectOfType<DemoSceneValidator>();
