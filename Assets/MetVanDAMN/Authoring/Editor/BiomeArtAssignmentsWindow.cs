	...
	public class BiomeArtAssignmentsWindow : EditorWindow
		...
		private void OnGUI()
			...
			if (Application.isPlaying && World.DefaultGameObjectInjectionWorld != null)
				...
				if (_live.Count == 0)
					...
					{
					foreach (KeyValuePair<BiomeType, Dictionary<string, int>> kv in _live.OrderBy(k => k.Key.ToString()))
						{
						...
						foreach (KeyValuePair<string, int> p in kv.Value.OrderByDescending(p => p.Value))
							{
		...
		private void DrawLibrarySummary()
			...
			if (lib.perTypeBuckets != null && lib.perTypeBuckets.Length > 0)
				{
				foreach (BiomeArtProfileLibrary.BiomeTypeBucket bucket in lib.perTypeBuckets)
					{
