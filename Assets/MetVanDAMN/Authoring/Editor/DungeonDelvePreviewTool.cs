...
namespace TinyWalnutGames.MetVanDAMN.Authoring.Editor
	...
	public class DungeonDelvePreviewTool : EditorWindow
		...
		private readonly Color[] floorColors =
			{
			new(0.5f, 0.8f, 1f, 0.7f), // Crystal blue
			new(1f, 0.4f, 0.2f, 0.7f), // Molten red
			new(0.6f, 0.2f, 0.9f, 0.7f) // Void purple
			};
		...
		[MenuItem("Tiny Walnut Games/MetVanDAMN!/Dungeon Delve Preview Tool")]
		...
		private void DrawHeader()
			...
			EditorGUILayout.LabelField("Preview and validate dungeon generation with specific seeds",
		...
		private void DrawPickups(Rect rect)
			...
			foreach (var pickup in currentPreview.pickups)
				...
				if (showLabels)
					...
					var ts = pickup.type.ToString();
					GUI.Label(labelRect, ts.Length > 0 ? ts[..1] : string.Empty, labelStyle);
					}
				}
			}
		...
		private void DrawValidationResults()
			...
			bool allValidationsPassed =
				hasThreeFloors && hasCorrectBossCount && hasCorrectLockCount && hasMinimumSecrets;

			EditorGUILayout.Space();

			if (allValidationsPassed)
				{
				EditorGUILayout.HelpBox("✅ All validations passed! This seed generates a valid dungeon.",
					MessageType.Info);
				}
			else
				{
				EditorGUILayout.HelpBox("⚠️ Some validations failed. Consider using a different seed.",
					MessageType.Warning);
				}
		...
		private Vector2 WorldToScreenPosition(Vector3 worldPos, Rect screenRect)
			...
			float y = ((worldPos.z / 30f) % 1f) * (screenRect.height / 3f) +
			          (worldPos.z / 30f) * (screenRect.height / 3f) + previewOffset.y;
