    ...
    public class DemoCombatManager : MonoBehaviour
        ...
        public class DemoInventoryManager : MonoBehaviour
            ...
            private void AutoSave()
                ...
                var playerInventory = FindFirstObjectByType<DemoPlayerInventory>();
        ...
        public class DemoRoomMaskingManager : MonoBehaviour
            ...
            private void InitializeRoomLayerMappings()
                ...
                var roomIdentifiers = FindObjectsByType<RoomIdentifier>(FindObjectsSortMode.None);
            ...
            private GameObject[] FindRoomObjects(int roomId)
                ...
                foreach (var obj in allObjects)
                    ...
                    if (((1 << obj.layer) & roomLayers) != 0)
                        ...
                        var foundRoomIdentifier = FindRoomIdentifier(roomId);
                        if (foundRoomIdentifier != null)
                            {
                            var roomBounds = new Bounds(foundRoomIdentifier.transform.position, foundRoomIdentifier.roomBounds);
            ...
            private List<int> GetAdjacentRooms(int roomId)
                ...
                var allRooms = FindObjectsByType<RoomIdentifier>(FindObjectsSortMode.None);
            ...
            private RoomIdentifier FindRoomIdentifier(int roomId)
                {
                var rooms = FindObjectsByType<RoomIdentifier>(FindObjectsSortMode.None);
            ...
            private void TriggerRoomEvents(int roomId, bool entering)
                ...
                foreach (var behaviour in behaviours)
                    ...
                    if (behaviour is IRoomEventListener listener)
                        {
                        if (entering) listener.OnRoomEnter(roomId); else listener.OnRoomExit(roomId);
                        }
                    }
        ...
        public class DemoRoomTransitionDetector : MonoBehaviour
            ...
            private void HandleRoomTransition()
                ...
                var maskingManager = FindFirstObjectByType<DemoRoomMaskingManager>();
                if (maskingManager)
                    {
            ...
            private void UpdatePlayerRoomState(int roomId)
                ...
                var mapGenerator = FindFirstObjectByType<MetVanDAMNMapGenerator>();
                if (mapGenerator)
                    {
        ...
        public class DemoValidationSettings : MonoBehaviour
            ...
            private bool ValidateSystemIntegration()
                ...
                var player = FindFirstObjectByType<DemoPlayerMovement>();
                var combat = FindFirstObjectByType<DemoPlayerCombat>();
                if (player && combat)
                    {
                ...
                var inventory = FindFirstObjectByType<DemoPlayerInventory>();
                if (inventory && combat)
                    {
                ...
                var aiManager = FindFirstObjectByType<DemoAIManager>();
                var lootManager = FindFirstObjectByType<DemoLootManager>();
            ...
            private bool ValidatePerformance()
                ...
                // Check object counts
                var totalGameObjects = FindObjectsByType<GameObject>(FindObjectsSortMode.None).Length;
                var rendererCount = FindObjectsByType<Renderer>(FindObjectsSortMode.None).Length;
                var colliderCount = FindObjectsByType<Collider>(FindObjectsSortMode.None).Length + FindObjectsByType<Collider2D>(FindObjectsSortMode.None).Length;
            ...
            private bool ValidateCompliance()
                ...
                if (complianceValidatorType != null)
                    ...
                    foreach (var b in behaviours)
                        {
                        if (complianceValidatorType.IsInstanceOfType(b)) { complianceValidatorInstance = b; break; }
                        }
                    var result = complianceValidatorType.GetMethod("ValidateCompliance");
