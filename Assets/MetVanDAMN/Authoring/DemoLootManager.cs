...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DemoLootManager : MonoBehaviour
		{
		[Header("Loot Settings")] public LootTable[] lootTables = System.Array.Empty<LootTable>();

		public GameObject[] treasureChestPrefabs = System.Array.Empty<GameObject>();
		public GameObject? pickupPrefab;

		[Header("Drop Chances")] [Range(0f, 1f)]
		...
		[Header("Treasure Spawning")] public int maxTreasureChests = 5;
		...
		public void SpawnTreasureChest(Vector3 position)
			...
			DemoTreasureChest chest = chestObj.GetComponent<DemoTreasureChest>();
			if (!chest)
				{
		...
		private DemoItem? RollForLoot(LootTable table)
			...
			foreach (LootItem lootItem in table.items)
				{
		...
		private void CreateLootPickup(DemoItem item, Vector3 position)
			...
			DemoLootPickup pickupComponent = pickup.GetComponent<DemoLootPickup>();
			if (!pickupComponent)
				{
