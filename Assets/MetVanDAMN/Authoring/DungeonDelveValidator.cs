...
using static TinyWalnutGames.MetVanDAMN.Authoring.DemoCombatManager;
...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DungeonDelveValidator : MonoBehaviour
		...
		private uint[] testSeeds = { 42, 123, 456, 789, 999 };

		public int validationIterations = 3;
		[SerializeField] private float maxGenerationTime = 5f;
		[SerializeField] private float targetFrameRate = 60f;

		// Validation results
		private ValidationReport lastReport;
		private List<ValidationReport> allReports = new();
...
#nullable enable
		...
		// Touch serialized fields to appease analyzers without altering behavior
		private void OnValidate()
			{
			_ = validationIterations; // touched by ValidateGenerationLogic
			}
		...
		public ValidationReport ValidateAll()
			...
			Debug.Log(
				$"✅ Validation completed in {lastReport.totalValidationTime:F2} seconds. Overall result: {(lastReport.overallPassed ? "PASSED" : "FAILED")}");
		...
		private void FindRequiredComponents()
			{
			dungeonMode = FindFirstObjectByType<DungeonDelveMode>();
			mainMenu = FindFirstObjectByType<DungeonDelveMainMenu>();
		...
		private void ValidateComponentPresence()
			...
			int foundPrefabs = 0;
			foreach (string prefabKey in requiredPrefabs)
				{
				// Check if prefab exists in ECS Prefab Registry
				EcsPrefabRegistryAuthoring registries = FindAnyObjectByType<EcsPrefabRegistryAuthoring>();
				bool prefabFound = false;

				if (registries != null)
					foreach (EcsPrefabRegistryAuthoring.Entry registry in registries.Entries)
						{
						if (registries.Entries != null)
							{
							foreach (EcsPrefabRegistryAuthoring.Entry entry in registries.Entries)
								{
				...
				if (!prefabFound)
					{
					Object resourcePrefab = Resources.Load($"Prefabs/{prefabKey}");
		...
		private void ValidateBossPrefabs()
			{
			string[] requiredBosses = new string[] { "Crystal Guardian", "Magma Serpent", "Void Overlord" };

			foreach (string bossName in requiredBosses)
				{
		...
		private void ValidateLockPrefabs()
			{
			string[] requiredLocks = new string[] { "Crystal Key", "Flame Essence", "Void Core" };

			foreach (string lockName in requiredLocks)
				{
		...
		private void ValidatePickupPrefabs()
			{
			System.Array requiredPickupTypes = System.Enum.GetValues(typeof(PickupType));
		...
		private void ValidateCombatIntegration()
			...
			DemoCombatManager combatManager = FindFirstObjectByType<DemoCombatManager>();
		...
		private void ValidateInventoryIntegration()
			...
			DemoInventoryManager inventoryManager = FindFirstObjectByType<DemoInventoryManager>();
		...
		private void ValidateMissingReferences()
			...
			foreach (MonoBehaviour component in allComponents)
				...
				FieldInfo[] fields = component.GetType().GetFields(System.Reflection.BindingFlags.Public |
				...
				foreach (FieldInfo field in fields)
					...
					bool isPublicUnityObject =
						field.IsPublic && typeof(UnityEngine.Object).IsAssignableFrom(field.FieldType);
			...
			if (nullReferences == 0)
				...
				{
				foreach (string detail in nullReferenceDetails)
					{
		...
		private void ValidateBiomeAssets()
			{
			string[] requiredBiomes = new string[] { "Crystal Caverns", "Molten Depths", "Void Sanctum" };

			foreach (string biomeName in requiredBiomes)
				{
		...
		private void ValidateGenerationLogic()
			...
			for (int i = validationIterations; i > 0; i--)
				{
				foreach (uint testSeed in testSeeds)
					{
					Debug.Log($"🔄 Iteration {i}/{validationIterations} for seed {testSeed}");
					ValidateGenerationWithSeed(testSeed);
					}
				}

			// Intentionally not modifying validationIterations; keep it as configured
			// Test edge cases
			ValidateEdgeCases();
			}
		...
		private void ValidateGenerationWithSeed(uint seed)
			...
			try
				...
				if (floorsGenerated && bossesPlaced && locksPlaced && secretsPlaced &&
		...
		private bool ValidateFloorGeneration(uint seed, Unity.Mathematics.Random random)
			...
			try
				...
				DungeonDelveMode tempDungeon = tempGO.AddComponent<DungeonDelveMode>();
		...
		private void ValidateEdgeCases()
			...
			foreach (uint seed in edgeCaseSeeds)
				{
				try
					{
		...
		private void ValidateUIElements()
			{
			string[] requiredUIElements = new[]
				{
				"Button", "Text", "Image", "Canvas"
				};

			var foundElements = new Dictionary<string, int>();
			foreach (string elementType in requiredUIElements)
				{
			...
			foreach (KeyValuePair<string, int> kvp in foundElements)
				{
				if (kvp.Value == 0)
					{
		...
		private void ValidateButtonFunctionality()
			...
			foreach (Button button in buttons)
				...
				if (button.onClick != null && button.onClick.GetPersistentEventCount() > 0)
					...
					System.Type targetType = typeof(UnityEngine.Events.UnityEvent);
					FieldInfo field = targetType.GetField("m_Calls",
						System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
					if (field != null)
						{
						object calls = field.GetValue(button.onClick);
						if (calls != null)
							{
							FieldInfo countField = calls.GetType().GetField("m_RuntimeCalls",
								System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
							if (countField != null)
								{
			...
			if (totalButtons > 0)
				...
				if (functionalPercentage >= 0.8f) // 80% of buttons should be functional
					...
					lastReport.passedChecks.Add(
						$"✓ Button functionality validated ({functionalButtons}/{totalButtons} functional)");
					}
				else
					{
					lastReport.failedChecks.Add(
						$"❌ Insufficient button functionality ({functionalButtons}/{totalButtons} functional)");
					}
				}
			else
				{
		...
		private void ValidatePickupEffects()
			{
			System.Array pickupTypes = System.Enum.GetValues(typeof(PickupType));
		...
		private void ValidatePerformance()
			...
			if (performanceAcceptable)
				...
				{
				lastReport.failedChecks.Add(
					$"❌ Performance test failed: {actualFrameRate:F1} fps (target: {targetFrameRate} fps)");
		...
		private void ValidateNarrativeCoherence()
			...
			lastReport.narrativeTestPassed =
				hasCoherentArc && biomesProgressive && bossesThemed && progressionMeaningful;
		...
		private bool ValidateStoryArc()
			...
			string[] surfaceKeywords = new[] { "crystal", "cavern", "surface", "entry" };
			string[] undergroundKeywords = new[] { "molten", "depth", "fire", "lava", "underground" };
			string[] otherworldKeywords = new[] { "void", "sanctum", "beyond", "otherworldly", "final" };
				...
				switch (i)
					{
						case 0: // Crystal biome
							...
							break;
						case 1: // Molten biome
							bossMatchesBiome = bossNames[i].ToLower().Contains("magma") ||
							                   bossNames[i].ToLower().Contains("fire");
							break;
						case 2: // Void biome
							bossMatchesBiome = bossNames[i].ToLower().Contains("void") ||
							                   bossNames[i].ToLower().Contains("overlord");
							break;
					}
	...
	public class ValidationReport
		{
		[Header("Overall Results")] public bool overallPassed;

		public float validationStartTime;
		public float validationEndTime;
		public float totalValidationTime;

		[Header("Component Validation")] public bool hasRequiredComponents;
		...
		[Header("System Integration")] public bool ecsIntegrationWorking;
		...
		[Header("Asset Validation")] public bool biomeAssetsComplete;

		public bool audioAssetsPresent;
		public bool materialAssetsPresent;

		[Header("Generation Testing")] public int seedGenerationTests;

		[Header("UI Validation")] public bool uiFullyFunctional;

		[Header("Gameplay Systems")] public bool progressionSystemWorking;

		public bool rewardSystemFunctional;

		[Header("Quality Assurance")] public bool performanceAcceptable;

		public bool narrativeTestPassed;

		[Header("Detailed Results")] public List<string> passedChecks = new();
