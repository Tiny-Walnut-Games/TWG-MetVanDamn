...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class UpgradeDefinition : ScriptableObject
		...
		[Header("Effects")] [SerializeField] private Ability grantsAbilities = Ability.None;
		...
		[Header("Rarity & Weighting")] [SerializeField] [Range(0f, 1f)]
		...
		[Header("Preview Text")] [SerializeField] [TextArea(1, 3)]
		...
		public bool IsAvailableFor(int playerLevel, Ability currentAbilities, string[] currentUpgradeIds)
			...
			foreach (string requiredId in requiredUpgradeIds)
				{
				bool hasRequired = false;
				foreach (string currentId in currentUpgradeIds)
					{
			...
			foreach (string conflictingId in conflictingUpgradeIds)
				{
				foreach (string currentId in currentUpgradeIds)
					{
