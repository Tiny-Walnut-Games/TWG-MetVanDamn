...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class LevelUpChoiceSystem : MonoBehaviour
		{
		[Header("Choice Generation")] [SerializeField] [Range(2, 6)]
		private int choicesPerLevelUp = 3;

		[SerializeField] private int minDistinctCategories = 2;
		[SerializeField] private bool preventDuplicateChoices = true;
		[SerializeField] private int maxRetries = 10;

		[Header("Upgrade Collections")] [SerializeField]
		private UpgradeCollection[] upgradeCollections = new UpgradeCollection[0];

		[Header("Weighting Configuration")] [SerializeField]
		private CategoryWeightEntry[] categoryWeights = new CategoryWeightEntry[0];
		...
		private Dictionary<UpgradeCategory, UpgradeCollection> collectionMap = new();
		...
		private void OnValidate()
			...
			UpgradeCategory[] categoriesWithCollections = upgradeCollections
				.Where(c => c != null)
				.Select(c => c.Category)
				.Distinct()
				.ToArray();

			UpgradeCategory[] allCategories =
				System.Enum.GetValues(typeof(UpgradeCategory)).Cast<UpgradeCategory>().ToArray();
			UpgradeCategory[] missingCategories = allCategories.Except(categoriesWithCollections).ToArray();
		...
		// Note: Unsubscription from OnLevelUp intentionally omitted; MonoBehaviour lifecycle will clear delegates on destroy.
		// If future memory profiling indicates retained references, reintroduce guarded unsubscription.
		...
		private void BuildCollectionMap()
			...
			if (enableDebugLogging)
				...
				foreach (KeyValuePair<UpgradeCategory, UpgradeCollection> kvp in collectionMap)
					{
					(int total, int unique, int available) = kvp.Value.GetStats(1, Ability.None, new string[0]);
					Debug.Log($"  • {kvp.Key}: {total} total, {unique} unique");
					}
				}
			}
		...
		public void GenerateUpgradeChoices()
			...
			// Generate choices
			UpgradeDefinition[] choices =
				GenerateChoices(currentLevel, currentAbilities, currentUpgrades, worldSeed, biomeContext);
		...
		private UpgradeDefinition[] GenerateChoices(int playerLevel, Ability currentAbilities, string[] currentUpgrades,
			...
			while (choices.Count < choicesPerLevelUp && attempts < maxRetries)
				...
				UpgradeCategory targetCategory =
					SelectCategory(usedCategories, currentAbilities, biomeContext, random.NextUInt());

				if (!collectionMap.TryGetValue(targetCategory, out UpgradeCollection? collection))
					{
				...
				List<UpgradeDefinition> availableUpgrades =
					collection.GetAvailableUpgrades(playerLevel, currentAbilities, currentUpgrades);
				...
				UpgradeDefinition selectedUpgrade = collection.SelectRandomUpgrade(playerLevel, currentAbilities,
					currentUpgrades,
					biomeContext, random.NextUInt());

				if (selectedUpgrade != null &&
				    (!preventDuplicateChoices || !usedUpgradeIds.Contains(selectedUpgrade.Id)))
					{
		...
		private UpgradeCategory SelectCategory(HashSet<UpgradeCategory> usedCategories, Ability currentAbilities,
			...
			foreach (UpgradeCategory category in System.Enum.GetValues(typeof(UpgradeCategory)).Cast<UpgradeCategory>())
				{
			...
			if (availableCategories.Count == 0)
				...
				foreach (UpgradeCategory category in collectionMap.Keys)
					{
					availableCategories.Add(category);
					weights.Add(CalculateCategoryWeight(category, currentAbilities, biomeContext, seed));
					}
		...
		private float CalculateCategoryWeight(UpgradeCategory category, Ability currentAbilities, Polarity biomeContext,
			uint seed)
			{
			float weight = 1f;

			// Base category weight
			CategoryWeightEntry? categoryWeightEntry = categoryWeights.FirstOrDefault(cw => cw.category == category);
			if (categoryWeightEntry != null)
				...
				// Biome influence
				if (collectionMap.TryGetValue(category, out UpgradeCollection? collection))
					{
					float biomeWeight = collection.CalculateCategoryWeight(biomeContext);
					weight *= Unity.Mathematics.math.lerp(1f, biomeWeight * categoryWeightEntry.biomeMultiplier,
						biomeInfluence);
					}
				}
		...
		private UpgradeCategory SelectWeightedCategory(List<UpgradeCategory> categories, List<float> weights,
		...
		private uint GetWorldSeed()
			...
			if (Unity.Entities.World.DefaultGameObjectInjectionWorld != null)
				...
				EntityQuery query = entityManager.CreateEntityQuery(typeof(WorldConfiguration));

				if (!query.IsEmptyIgnoreFilter)
					{
					WorldConfiguration config = query.GetSingleton<WorldConfiguration>();
		...
		private Polarity GetCurrentBiomeContext()
			{
			// Safe due to earlier null checks before calling this method; use null-forgiving operator
			Transform playerTransform = playerProgression!.transform;
			...
			// worldSeed is a prefs value set at world generation time
			uint worldSeed = GetWorldSeed();
			if (worldSeed != 0)
				{
