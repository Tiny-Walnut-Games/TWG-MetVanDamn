...
#nullable enable
	...
	public class DemoAIManager : MonoBehaviour
		{
		[Header("AI Settings")] public GameObject[] enemyPrefabs;
	...
	public class DemoEnemyAI : MonoBehaviour, IDemoDamageable
		...
		[Header("Patrol Settings")] public float patrolRadius = 5f;
		...
		private void OnDestroy()
			{
			if (navigationInitialized && defaultWorld != null && defaultWorld.IsCreated &&
			    entityManager.Exists(navigationEntity))
				{
		...
		private void FollowNavigationPath(float speed)
			...
			else if (navState.Status == PathfindingStatus.NoPathFound ||
			         navState.Status == PathfindingStatus.TargetUnreachable)
				{
				// Fallback to direct movement
				if (target != null)
		...
		private void TryRangedAttack()
			...
			if (projectilePrefab)
				...
				GameObject projectile = Instantiate(projectilePrefab, transform.position + direction * 0.5f,
					Quaternion.identity);
