...
namespace TinyWalnutGames.MetVanDAMN.Authoring
	...
	public class DemoPlayerCombat : MonoBehaviour
		...
		[Header("Combat Settings")] public int maxHealth = 100;
		...
		[Header("Input")] public KeyCode lightAttackKey = KeyCode.Mouse0;
		...
		[Header("Audio")] public AudioClip attackSound;
		...
		private void HandleCombatInput()
			...
			if (Input.GetKeyDown(lightAttackKey))
				{
				if (isCharging) ReleaseChargedAttack();
		...
		private void PerformLightAttack()
			...
			DemoWeapon weapon = GetCurrentWeapon();
		...
		private void ExecuteMeleeAttack(DemoWeapon weapon, float damage, AttackType attackType)
			...
			foreach (Collider hit in hits3D)
				{
				DamageTarget(hit.gameObject, damage, attackType);
				}
		...
		private void ExecuteRangedAttack(DemoWeapon weapon, float damage, AttackType attackType)
			...
			if (weapon.projectilePrefab == null)
				...
				Projectile? projectileScript = projectile.TryGetComponent<Projectile>(out Projectile? ps) ? ps : null;
				// pool projectiles
			...
			for (int i = 0; i < projectileCount; i++)
				...
				if (weapon.projectilePrefab)
					...
					Projectile? projectileScript =
						projectile.TryGetComponent<Projectile>(out Projectile? ps) ? ps : null;
					if (projectileScript != null)
						{
		...
		private void DamageTarget(GameObject target, float damage, AttackType attackType)
			...
			IDemoDamageable enemy = target.GetComponent<IDemoDamageable>();
		...
		private void CreateAttackEffect(Vector3 position, float radius, Color color)
			...
			AttackEffectPulse pulseAnimation = effect.AddComponent<AttackEffectPulse>();
		...
		private void TriggerScreenShake(float intensity)
			...
			foreach (Camera? camera in cameras)
				{
				if (!camera.TryGetComponent<CameraShake>(out CameraShake? shakeComponent))
					{
		...
		public DemoWeapon GetCurrentWeapon() =>
			availableWeapons.Count == 0 ? SentinelWeapon : availableWeapons[currentWeaponIndex];
		...
		/// </summary>
		public void SetStats(int newMaxHealth, float newAttackDamage, float newAttackSpeed, float newCriticalChance,
			float newDamageReduction)
			{
