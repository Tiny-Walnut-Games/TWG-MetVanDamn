...
namespace TinyWalnutGames.MetVD.QuadRig.Authoring
	...
	public class QuadRigHumanoidAuthoring : MonoBehaviour
		{
		[Header("Character Configuration")] [SerializeField]
		private uint rigId = 1;
		...
		private float billboardRotationSpeed = 5.0f;

		[Header("Biome Atlas Configuration")] [SerializeField]
		private uint startingAtlasId = 0;

		[SerializeField] private BiomeSkinUtility.BiomeType biomeType = BiomeSkinUtility.BiomeType.Default;
		[SerializeField] private Vector2Int atlasDimensions = new(512, 512);

		[Header("Character Parts")] [SerializeField]
		private bool autoGenerateParts = true;

		[SerializeField] private QuadPartConfiguration[] customParts;

		[Header("Debug & Testing")] [SerializeField]
		private bool enableStoryTest = false;
		...
		private void DrawStandardPartGizmos()
			...
			foreach (QuadPartType partType in System.Enum.GetValues(typeof(QuadPartType)))
				{
		...
		private void RunStoryTest()
			{
			if (!TryGetComponent<QuadRigStoryTest>(out QuadRigStoryTest? storyTest))
				{
		...
		public class QuadPartConfiguration
			...
			public Rect uvRect = new(0, 0, 0.25f, 0.25f);
			}
		...
		public class QuadRigHumanoidBaker : Baker<QuadRigHumanoidAuthoring>
			...
			private void CreateStandardParts(Entity entity, QuadRigHumanoidAuthoring authoring)
				...
				foreach (var part in standardParts)
					...
					AddComponent(partEntity, new Parent { Value = entity });
					}
				}
			...
			private void CreateCustomParts(Entity entity, QuadRigHumanoidAuthoring authoring)
				...
				foreach (var partConfig in authoring.customParts)
					...
					var meshPart = new QuadMeshPart(
						partConfig.partType,
						new float4(partConfig.uvRect.x, partConfig.uvRect.y, partConfig.uvRect.width,
							partConfig.uvRect.height),
